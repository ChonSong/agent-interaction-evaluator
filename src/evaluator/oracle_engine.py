...
from typing import Any, Dict as _Dict, List as _List
...
class Condition:
    ...
    op: str = "gt"
    condition_cfg: _Dict = _field(default_factory=dict)
    events_list: _List = _field(default_factory=list)
    min_length: int | None = None
    max_length: int | None = None
    pattern: str = ""
    values: _List = _field(default_factory=list)

    @classmethod
    def from_dict(cls, data: dict) -> "Condition":
        known = {"type","field","value","reference","threshold","threshold_value","op","condition_cfg","events_list","min_length","max_length","pattern","values"}
...
class Oracle:
    ...
    trigger: str = ""
    severity: str = ""
    conditions: _List = _field(default_factory=list)
    actions: _List = _field(default_factory=list)
    ...
    def from_dict(cls, data: dict) -> "Oracle":
        return cls(
            ...
            name=data.get("name", data.get("oracle_id","")),
...
class ConditionResult:
    oracle_id: str = ""
    passed: bool = False
    failed_conditions: _List = _field(default_factory=list)
...
class FieldRequiredEvaluator:
    ...
    def evaluate(event: dict, field: str, **kwargs) -> bool:
        ...
        # Support structured value: { "not_empty": true }
        v = kwargs.get("value")
        if isinstance(v, dict) and v.get("not_empty"):
...
class FieldMinLengthEvaluator:
    ...
    def evaluate(event: dict, field: str, value: int = 0, **kwargs) -> bool:
        ...
        if isinstance(length_threshold, dict):
            length_threshold = length_threshold.get("min_length", 0)
        if not isinstance(length_threshold, (int, float)):
            length_threshold = kwargs.get("min_length", 0) or 0
        val = _get_nested(event, field)
        if not isinstance(val, str): return False
        return len(val) >= length_threshold

class FieldMaxLengthEvaluator:
    TYPE = "field_max_length"
    @staticmethod
    def evaluate(event: dict, field: str, value: int = 0, **kwargs) -> bool:
        length_threshold = value or kwargs.get("max_length", float("inf")) or float("inf")
        val = _get_nested(event, field)
...
class FieldRegexEvaluator:
    ...
    def evaluate(event: dict, field: str, value: str = "", **kwargs) -> bool:
        val = _get_nested(event, field)
        if not isinstance(val, str): return False
        pattern = value or kwargs.get("pattern", "") or ""
        if not pattern: return True  # no pattern to match = pass
        try:
...
class FieldInEvaluator:
    TYPE = "field_in"
    @staticmethod
    def evaluate(event: dict, field: str, values: _List = None, **kwargs) -> bool:
...
class DriftScoreThresholdEvaluator:
    ...
    def evaluate(event: dict, threshold_value: float = 0.0, op: str = "gt", **kwargs) -> bool:
        ...
        try:
            ...
            if op == "gt": return ds > tv
            elif op == "lt": return ds < tv
            elif op == "gte": return ds >= tv
            elif op == "lte": return ds <= tv
            elif op == "eq": return ds == tv
            return False
        except: return False
...
class RatioThresholdEvaluator:
    ...
    @staticmethod
    def evaluate(events: _List, condition: _Dict = None, threshold_value: float = 0.0, op: str = "gt", **kwargs) -> bool:
        ...
        tv = float(threshold_value)
        if op == "gt": return ratio > tv
        elif op == "lt": return ratio < tv
        elif op == "gte": return ratio >= tv
        elif op == "lte": return ratio <= tv
        elif op == "eq": return ratio == tv
        return False
...
_EVALUATOR_CLASSES = {
    "field_required": FieldRequiredEvaluator,
    "field_not_empty": FieldNotEmptyEvaluator,
    "field_min_length": FieldMinLengthEvaluator,
    "field_max_length": FieldMaxLengthEvaluator,
    "field_regex": FieldRegexEvaluator,
    "field_eq": FieldEqEvaluator,
    "field_gt": FieldGtEvaluator,
    "field_lt": FieldLtEvaluator,
    "field_in": FieldInEvaluator,
    "similarity_threshold": type("S",(),{"TYPE":"similarity_threshold"})(),
    "drift_score_threshold": type("D",(),{"TYPE":"drift_score_threshold"})(),
    "ratio_threshold": type("R",(),{"TYPE":"ratio_threshold"})(),
}
...
def _eval_cond(cond: Condition, event: dict) -> bool:
    ...
    kwargs = {
        ...
        "condition": cond.condition_cfg,
        "events": cond.events_list,
        "min_length": getattr(cond, "min_length", None),
        "max_length": getattr(cond, "max_length", None),
        "pattern": getattr(cond, "pattern", None),
        "values": getattr(cond, "values", None),
    }
    if isinstance(ec, type) and issubclass(ec, SimilarityThresholdEvaluator):
        return SimilarityThresholdEvaluator().evaluate(event, **kwargs)
    try:
...
def evaluate_conditions(oracle: Oracle, event: dict) -> ConditionResult:
    ...
    return ConditionResult(
        ...
        deviation="; ".join(deviations) if deviations else None,
        evaluated_at=get_current_timestamp(),
    )

def apply_actions(oracle: Oracle, event: dict, result: ConditionResult) -> None:
    ...
    if severity in ("critical", "warning"):
        try:
            t = threading.Thread(target=lambda: asyncio.run(_insert_result(result, event)), daemon=True)
            t.start()
        except Exception:
            pass
    if severity in ("critical", "warning"):
        ...
        try:
            t = threading.Thread(target=lambda: asyncio.run(_send_alert(msg, severity)), daemon=True)
            t.start()
        except Exception:
            pass
    if severity == "critical":
        try:
            t = threading.Thread(target=lambda: asyncio.run(_emit_cb(oracle, event, result)), daemon=True)
            t.start()
        except Exception:
            pass
...
async def _emit_cb(oracle, event, result):
    ...
    cb = {
        ...
        "agent_id": event.get("agent_id", "aie-system"),
        "session_id": event.get("session_id", "unknown"),
        "schema_version": "1.0",
        "interaction_context": event.get("interaction_context", {}),
        "gate": {
            "name": oracle.oracle_id,
            "threshold": result.deviation or "oracle_failed",
            "assumptions_violated": [event.get("event_id")],
        },
        "action_blocked": None,
        "halt_session": True,
        "alert_sent": True,
        "audit_ref": f"oracle_result:{oracle.oracle_id}",
    }
...
class OracleRegistry:
    def __init__(self):
        self._oracles: _Dict[str, Oracle] = {}
        ...
        for yf in base.rglob("*.yaml"):
            ...
            try:
                ...
                if "oracle_id" in data:
                    o = Oracle.from_dict(data)
                    self._oracles[o.oracle_id] = o
                    count += 1
                elif isinstance(data.get("oracles"), list):
                    for item in data["oracles"]:
                        o = Oracle.from_dict(item)
                        self._oracles[o.oracle_id] = o
                        count += 1
            except:
                pass
    ...
    def get_for_event_type(self, et: str) -> _List[Oracle]:
        return [o for o in self._oracles.values() if o.event_type == et]

    def get(self, oid: str) -> Oracle | None:
        return self._oracles.get(oid)

    def list(self) -> _List[Oracle]:
        return list(self._oracles.values())

    def validate(self, oid: str) -> tuple[bool, str | None]:
        o = self._oracles.get(oid)
        if o is None:
            return False, f"Oracle '{oid}' not found"
        errs = []
        if not o.conditions:
            errs.append("Must have at least one condition")
        for c in o.conditions:
            if c.type not in _EVALUATOR_CLASSES:
                errs.append(f"Unknown condition type: '{c.type}'")
        if o.severity not in ("critical", "warning", "info"):
            errs.append(f"Invalid severity: '{o.severity}'")
        if o.trigger not in ("on_event", "on_demand", "on_cron"):
            errs.append(f"Invalid trigger: '{o.trigger}'")
        return (False, "; ".join(errs)) if errs else (True, None)

    def validate_all(self) -> _Dict:
        return {oid: self.validate(oid) for oid in self._oracles}

def evaluate_event(event: dict, registry: OracleRegistry | None = None) -> _List[ConditionResult]:
    import asyncio
    if registry is None:
